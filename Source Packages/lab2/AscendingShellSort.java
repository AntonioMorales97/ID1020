 ...
 * ascending order using the shellsort algorithm and prints out the whole array
 * in each iteration of the inner loop.
 * 
 * @author Antonio
 *
 */
public class AscendingShellSort {
     ...
     * (insertion sort) the partially sorted array.
     * 
    ...
    public static void shellSort(Comparable[] array) {
        ...
        while (interval > 0) {
            ...
            for (int i = interval; i < array.length; i++) {
                for (int j = i; j >= interval && less(array[j], array[j - interval]); j = j - interval) {
                    show(array);
